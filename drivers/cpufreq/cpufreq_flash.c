...
static ssize_t show_timer_slack(struct cpufreq_flash_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->timer_slack_val);
}

static ssize_t store_timer_slack(struct cpufreq_flash_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
					...
					struct cpufreq_policy *policy)
{
	...
	ppol->policy_slack_timer.function = cpufreq_flash_nop_timer;
